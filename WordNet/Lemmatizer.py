...
class Lemmatizer:
	...
	# Метод возвращает True если слово найдено в исключениях		
	def IsIrregular(self, word):
		# Если в слове есть тире, разделим слово на части, если хоть одна часть является исключением - всё считается исключением
		wordArr = word.split(self.splitter)
		resultWord = False
		for word in wordArr:
			lemma = self.__IsIrregularWord(word)
			if (lemma != None):
				resultWord=(resultWord or lemma)
		if (resultWord != None):
			return (resultWord)
		return None		
	...
	# Метод ищет в исключениях			
	def __IsIrregularWord(self, word):
		for item in self.wordNet:
			lemma = item.IsIrregular(word)
			if (lemma != None):
				return lemma
		return None		
